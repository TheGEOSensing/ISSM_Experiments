...
shp2exp('FR_ice_shelf_3031.shp','FR_ice_shelf.exp');

%% Mesh parameters

domain = '/Users/rishi/Desktop/ISSM_Project_Ross/Filchner_Ronne/FR_ice_shelf.exp';
%domain = '/Users/rishi/Desktop/ISSM_Project_Ross/PanAntarctica/AntarcticaExtended.exp';
hinit=5000;	% element size for the initial mesh
hmax=10000;		% maximum element size of the final mesh
hmin=3000;		% minimum element size of the final mesh
...
% Mouginot's data on INSAR ice velocity all Antarctica
...
% Model name
md.miscellaneous.name = 'FR_Inversion';

% Plot some parameters
plotmodel(md,'data','BC', ... % boundary conditions
    'data',md.inversion.vel_obs,'log#2',10,'caxis#2',[0.1 2000],... % observed velocity 
...
mds = extract(md,md.mask.ocean_levelset<0 & md.mask.ice_levelset<0); % only for the ice shelf
mds = solve(mds,'Stressbalance');
save FR_IceShelf_Inversion mds;
...
'caxis#1',[0 2000],'caxis#2',[0 2000],'caxis#4',[-100 100])


%% Caclulate the strain rates and stress information from the mechancial properties toolbox
...
mds = mechanicalproperties(mds, vx, vy); 

% will get the output in mds.results
...
'data',mds.results.deviatoricstress.yy,'title','transverse','caxis#all', [-2e5 2e5]);
...
%% Caclaute Buttressing Ratio and Backstress using Furst et al.,(2016) method

[Kn, Backstress_Furst, Integrated_Backstress] = Furst_buttressing(mds, vx, vy, false);
